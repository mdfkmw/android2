...
export default function ReservationPage({ userRole, user }) {
  ...
  const drawSeatMapCanvas = useCallback((driverName = '') => {
    ...
    seats.forEach((seat) => {
      ...
      const writeLine = (text, font = '12px "Inter", sans-serif', options = {}) => {
        if (!text) return;
        ...
        const alignX =
          textAlign === 'right'
            ...
            : textAlign === 'center'
              ? x + seatWidth / 2
              : x + seatPadding;
        ctx.fillText(value, alignX, textY);
        ctx.textAlign = 'left';
        textY += 16;
      };
      ...
      const writeObservation = (text) => {
        ...
        lines.forEach((line, idx) => {
          const prefix = idx === 0 ? '📝 ' : '   ';
          const observationFont = `italic ${diagramTextStyle.fontSize || 10}px ${diagramTextStyle.fontFamily || '"Inter", sans-serif'}`;
          writeLine(`${prefix}${line}`, observationFont, {
